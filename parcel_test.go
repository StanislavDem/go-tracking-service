...
func TestAddGetDelete(t *testing.T) {
	...
	_, err = store.Get(id)
	require.Error(t, err) // тут должна быть ошибка
	require.Equal(t, sql.ErrNoRows, err) // sql.ErrNoRows
}
...
func TestGetByClient(t *testing.T) {
	...
	// get by client
	storedParcels, err := NewParcelStore(db).GetByClient(client)// получите список посылок по идентификатору клиента, сохранённого в переменной client
